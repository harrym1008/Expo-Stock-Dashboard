...
export function formatStockQuote(item, liveQuote, liveProfile, y1D, marketStatus) {
  ...
  if (marketStatus?.isOpen) {
    ...
    const prePrice = y1D?.preMarketPrice || liveQuote?.preMarketPrice || item?.preMarketPrice;
    const postPrice = y1D?.postMarketPrice || liveQuote?.postMarketPrice || item?.postMarketPrice;
  ...
  const dynamicSparkline =
    displayPrice > 0 && baseSparkline.length > 0
  ...
  const currency = sec?.currency ?? item?.currency ?? '$';
  ...
  const resolvedPreMarket =
    y1D?.preMarketPrice ||
    liveQuote?.preMarketPrice ||
    (marketStatus?.isPreMarket && displayPrice > 0 ? displayPrice : null);

  // Post-market price: prefer 1D/live, else live price when fully closed
  const resolvedPostMarket =
    y1D?.postMarketPrice ||
    liveQuote?.postMarketPrice ||
    (!marketStatus?.isPreMarket && !marketStatus?.isOpen && displayPrice > 0 ? displayPrice : null);
  ...
  return {
    ...
    isStock,
    price: displayPrice,
    previousClose: prevDayClose > 0 ? prevDayClose : (item?.previousClose > 0 ? item.previousClose : null),
